...
func DSN(userstore bool) (*dsn.Info, func(), error) {
	info, err := dsn.NewInfoFromEnv("")
	if err != nil {
	...
	if err = SetupDB(info); err != nil {
		return nil, nil, fmt.Errorf("failed to setup database: %w", err)
	}

	fn := func() {
		if err := TeardownDB(info); err != nil {
...
type ConnectorCreator func(*dsn.Info) (driver.Connector, error)
...
// suffix `connector`.
func RegisterDSN(name string, info *dsn.Info, connectorFn ConnectorCreator) error {
